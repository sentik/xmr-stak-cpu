...
void Update(hashState *state, const BitSequence *data);
void Final(hashState *state, BitSequence *hashval);
...
#define round_function(nn,r)                                                              \
      SS(y0,y2,y4,y6,y1,y3,y5,y7, LOAD(E8_bitslice_roundconstant2[r]), LOAD(E8_bitslice_roundconstant2[r]+16) ); \
...
__forceinline void Init(hashState *state)
{
	/*initialize the initial hash value of JH*/
	/*load the intital hash value into state*/
	state->x0 = LOAD(JH256);
	state->x1 = LOAD(JH256 + 16);
	state->x2 = LOAD(JH256 + 32);
	state->x3 = LOAD(JH256 + 48);
	state->x4 = LOAD(JH256 + 64);
	state->x5 = LOAD(JH256 + 80);
	state->x6 = LOAD(JH256 + 96);
	state->x7 = LOAD(JH256 + 112);
}

/*hash each 512-bit message block, except the last partial block*/
__forceinline void Update(hashState *state, const BitSequence *data)
{
	for (int i = 0; (i + 512) <= 1600; i = i + 512) 
	{
		memcpy(state->buffer, data + (i >> 3), 64);
		F8(state);
	}

	//memcpy(state->buffer, data + 0, 64);
	//F8(state);
	//
	//memcpy(state->buffer, data + 64, 64);
	//F8(state);
	//
	//memcpy(state->buffer, data + 128, 64);
	//F8(state);

	memset(state->buffer, 0, 64);
	memcpy(state->buffer, data + 192, 8);
}

/*pad the message, process the padded block(s), truncate the hash value H to obtain the message digest*/
__forceinline void Final(hashState *state, BitSequence *hashval)
{
